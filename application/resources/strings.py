...
def from_cart_items(cart_items, language, total) -> str:
    ...
    for cart_item in cart_items:
        counter += 1
        if cart_item.dish.is_sale:
            dish_price = cart_item.dish.sale_price * currency_value
        else:
            dish_price = cart_item.dish.price * currency_value
        if language == 'uz':
            dish_item = cart_str_item.format(counter=counter,
                                             name=cart_item.dish.get_full_name_uz(),
                                             count=cart_item.count,
                                             price=_format_number(dish_price),
                                             sum=_format_number(cart_item.count * dish_price))
        else:
            dish_item = cart_str_item.format(counter=counter,
...
def from_order(order: Order, language: str, total: int) -> str:
    ...
    for order_item in order.order_items.all():
        ...
        if order_item.dish.is_sale:
            dish_price = order_item.dish.sale_price * currency_value
        else:
            dish_price = order_item.dish.price * currency_value
        if language == 'uz':
        ...
        order_item_str = order_item_tmpl.format(counter=counter,
                                                ...
                                                price=_format_number(dish_price),
                                                sum=_format_number(order_item.count * dish_price),
