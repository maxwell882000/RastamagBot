...
def _total_order_sum(order_items) -> int:
    ...
    for order_item in order_items:
        if order_item.dish.is_sale:
            total += order_item.dish.sale_price * order_item.count
        else:
            total += order_item.dish.price * order_item.count
    return total
...
def _total_cart_sum(cart) -> int:
    ...
    for cart_item in cart:
        if cart_item.dish.is_sale:
            total += cart_item.dish.sale_price * cart_item.count
        else:
            total += cart_item.dish.price * cart_item.count
    return total
